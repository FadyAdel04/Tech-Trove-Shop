  ...
  return (
    <div className='max-w-7xl mx-auto mt-20'>
      <div className='grid grid-cols-1 lg:grid-cols-2'>
        ...
        {/* Left - Main Image */}
        <div className="shadow-md relative h-120 overflow-hidden aspect-ratio-1">
          <img
            src={selectedImage}
            layout="fill"
            objectfit="cover"
          ...
          <div className="flex mt-6 space-x-3">
            {product?.colors?.map((color) => {
              switch (color) {
                ...
                case 'RED':
                  return <div onClick={()=>{setSelectedColor(color)}} key={color} className={`${color == selectedColor ? "border-4 border-[#5b20b6]":""} w-8 h-8 rounded-full bg-red-500 cursor-pointer hover:border-4 border-[#5b20b6]`}></div>;
                case 'Gold':
                  return <div onClick={()=>{setSelectedColor(color)}} key={color} className={`${color == selectedColor ? "border-4 border-[#5b20b6]":""} w-8 h-8 rounded-full bg-amber-300 cursor-pointer hover:border-4 border-[#5b20b6]`}></div>;
              }
            })}
          </div>
